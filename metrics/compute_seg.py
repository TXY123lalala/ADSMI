...
def get_args():
    ...
    parser.add_argument("--GT_maskDIR", type=str, default="/data3/xytan/PolypGen-Benchmark/results_polypGen_deeplabv3plus_resnet50_C1C3C4C5", help="ground truth mask image (5 channel tif image only)")
    parser.add_argument("--Eval_maskDIR", type=str, default="/data3/xytan/PolypGen-Benchmark/results_polypGen_deeplabv3plus_resnet50_C1C3C4C5", help="provide folder for testType1 dataset under that name")
...
#    os.makedirs(savefolder, exist_ok = True)
    ...
    pred_mask_files = glob.glob(os.path.join(fpath, '*_pred.png'))
    fnames.append(pred_mask_files)
        
    print('running endocv segmentation...')
    print(pred_mask_files)
    if len(pred_mask_files) > 0:
        gt_mask_files = np.hstack([os.path.join(GT_folder, (os.path.split(f)[-1].split('_')[0])+'_target.png') for f in pred_mask_files])
