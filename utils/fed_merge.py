...
def FedAvg_1(model_dict, weight_dict, global_model=None):
    new_model_dict = None
    for model_name in weight_dict.keys():
        model = model_dict[model_name]
        model_state_dict = model.state_dict()

        model_state_dict_update=collections.OrderedDict()
        for k,v in model_state_dict.items():
            if 'online' in k:
                model_state_dict_update[k] = v
                    
        if new_model_dict is None:
            new_model_dict = Dict_weight(model_state_dict_update, weight_dict[model_name])
        else:
            new_model_dict = Dict_Add(new_model_dict, Dict_weight(model_state_dict_update, weight_dict[model_name]))
...
def FedUpdate_1(model_dict, global_model):
    global_model_parameters = global_model.state_dict()
    for site_name in model_dict.keys():
        model_state_dict_update=collections.OrderedDict()
        for k,v in global_model_parameters.items():
            if 'online' in k:
                model_state_dict_update[k] = v
        model_dict[site_name].load_state_dict(model_state_dict_update, strict=False)
    return None
