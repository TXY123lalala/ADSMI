...
from ADA_data import StandardDataset
from ADA_data.JigsawLoader import JigsawDataset, AdvDataset, get_split_dataset_info, _dataset_info
...
def append_adversarial_samples(args, data_loader, adv_data, transform_ADA, shuffle):
    ...
    dataset_adv = AdvDataset(adv_data, transform_ADA)
    datasets.append(dataset_adv)

    dataset = ConcatDataset(datasets)
    loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=shuffle, num_workers=4, pin_memory=True, drop_last=True)
    return loader
