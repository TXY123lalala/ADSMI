...
class DropPositionMaskedAutoEncoderViT(nn.Module):
    ...
    def forward(self, imgs, img_mae=None, mask_ratio=0.75, pos_mask_ratio=0.75, smooth=0, aug = False):
        mask_ratio=0.75
        #pos_mask_ratio=0.75
        #smooth = _get_label_smoothing_map(256, 0.0)

        outs = self.forward_encoder(imgs, mask_ratio, pos_mask_ratio)
        pred, pred_mae = self.forward_decoder(outs)
        if self.multi_task:
        ...
        if aug:
            return outs['x'], self.forward_drop_pos_loss(pred, outs['mask'], outs['ids_keep'], outs['mask_pos'], smooth,
                                          attn=outs['attn'] if self.attn_guide else None), self.forward_mae_loss(img_mae, pred_mae, outs['mask'])
        else:
            return outs['x'], self.forward_drop_pos_loss(pred, outs['mask'], outs['ids_keep'], outs['mask_pos'], smooth,
                                          attn=outs['attn'] if self.attn_guide else None)
